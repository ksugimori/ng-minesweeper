...
describe('Field', () => {
  describe('#pointsArround', () => {
    it('周囲の 8 座標が返ってくること', () => {
      ...
      expect(result).toContain(Point.of(0, 0))
      expect(result).toContain(Point.of(0, 1))
      expect(result).toContain(Point.of(0, 2))

      expect(result).toContain(Point.of(1, 0))
      // x: 1, y: 1 が引数なのでこれは返ってこない
      expect(result).toContain(Point.of(1, 2))

      expect(result).toContain(Point.of(2, 0))
      expect(result).toContain(Point.of(2, 1))
      expect(result).toContain(Point.of(2, 2))
    })
